// src/lib/discord.ts - دالة موحدة للتعامل مع Discord API - محسّن v5
...
// Rate limit global tracker - كل طلب يبدأ من الصفر (serverless)
let globalRLUntil = 0;
let globalRLHits = 0;
      ...
      try {
        ...
        if (res.status === 429) {
          ...
          try {
            ...
            const errData = await res.json() as { retry_after?: number; [key: string]: any };
