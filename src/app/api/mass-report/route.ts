...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = rateLimit(`${ip}:mass-report`, RATE_LIMITS.heavy);
    if (rl.limited) {
      return NextResponse.json({ success: false, error: 'تم تجاوز الحد - حاول لاحقاً' }, { status: 429 });
    }

    const body = await request.json().catch(() => ({}));
    const { token, targetId, reason, count } = body;

    if (!token || !targetId) {
      return NextResponse.json({ success: false, error: 'أدخل التوكن وأيدي المستهدف' }, { status: 400 });
    }
