...
export async function POST(req: NextRequest) {
  try {
    const { email, password, captcha_key, captcha_rqtoken, mfa_code, mfa_ticket } = await req.json()

    if (!email || !password) {
      return NextResponse.json({ success: false, error: 'أدخل الإيميل والباسورد' })
    }
