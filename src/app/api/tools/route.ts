...
export async function POST(request: NextRequest) {
  try {
    ...
    const rl = rateLimit(`${ip}:tools`, RATE_LIMITS.default);
    if (rl.limited) {
    ...
    const message = e instanceof Error ? e.message : 'خطأ في الطلب';
    return NextResponse.json({ success: false, error: 'خطأ: ' + message }, { status: 500 })
  }
}
