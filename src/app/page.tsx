...
type Section = 'verify' | 'nuker' | 'copy' | 'spam' | 'leveling' | 'sniper' | 'checker' | 'multi-spam' | 'mass-dm' | 'leaver' | 'react' | 'webhook-spam'
...
interface Stats {
  ...
  blocked?: number; left?: number; total?: number
}

interface Result { username: string; status: string; color: string }
...
export default function Home() {
  ...
  const [sniperPassword, setSniperPassword] = useState('')
  ...
  const api = async (endpoint: string, body: any) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        else if (endpoint === 'leaver' && data.servers) { setExtraData(data.servers) }
        if (data.stats) setStats(data.stats)
        setLoading(false); setProgress(''); return data
      } else { setResult(`❌ ${data.error}`); setLoading(false); setProgress(''); return null }
    } catch { setResult('❌ خطأ في الاتصال'); setLoading(false); setProgress(''); return null }
  }
    ...
    <div className="min-h-screen relative">
      ...
      <header className="glass sticky top-0 z-50 border-b border-green-500/20 px-4 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <h1 className="text-xl font-bold text-green-400 flex items-center gap-2">
            ...
            <span className="text-xs text-green-600 bg-green-500/10 px-2 py-0.5 rounded-full border border-green-500/20 ml-1">v2.5</span>
          </h1>
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-green-500 bg-green-500/10 px-3 py-1 rounded-full border border-green-500/20">⚡ 12 ميزة</span>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto flex relative z-10">
        <aside className="w-64 min-h-screen glass-card p-3 hidden lg:block sticky top-[57px] self-start border-l border-green-500/10 overflow-auto max-h-[calc(100vh-57px)]">
          <div className="text-center mb-5 pb-4 border-b border-green-500/15"><div className="text-3xl mb-1">🛡️</div><h2 className="text-base font-black text-green-400">TRJ BOT</h2><p className="text-[10px] text-green-600 mt-0.5">v2.5 - محسّن و سريع</p></div>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            {section === 'copy' && (
              <div className="animate-fade-in"><div className="glass-card rounded-2xl p-6 border border-green-500/20 shadow-xl shadow-green-500/5">
                ...
                <p className="text-green-600 text-sm mb-5">نسخ سيرفر كامل بالتوازي - رتب + رومات + إعدادات</p>
            ...
            {section === 'sniper' && (
              <div className="animate-fade-in"><div className="glass-card rounded-2xl p-6 border border-green-500/20 shadow-xl shadow-green-500/5">
                <div className="flex items-center justify-between mb-1"><div className="flex items-center gap-3"><span className="text-2xl">🎯</span><h2 className="text-xl font-black text-green-400">صيد يوزرات (Pomelo)</h2></div>
                  <button onClick={async () => { if (!sniperToken) { setResult('❌ أدخل التوكن أولاً'); return }; setLoading(true); setProgress('🔍 جلب معلومات الحساب...'); try { const res = await fetch('/api/sniper', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: sniperToken, action: 'accountInfo' }) }); const data = await res.json(); if (data.success) { setSniperAccountInfo(data.info); setResult('') } else { setResult(`❌ ${data.error}`) } } catch { setResult('❌ خطأ في الاتصال') }; setLoading(false); setProgress('') }} className="text-xs text-cyan-400 bg-cyan-500/10 px-3 py-1.5 rounded-lg border border-cyan-500/20 hover:bg-cyan-500/20 transition-colors cursor-pointer">🔍 معلومات الحساب</button>
                </div>
                <p className="text-green-600 text-sm mb-5">⚡ نظام ذكي مع تجاوز Rate Limit + 5 أنماط!</p>
                {sniperAccountInfo && (<div className="mb-4 bg-cyan-500/5 rounded-xl p-4 border border-cyan-500/15 animate-fade-in">
                  ...
                  {sniperAccountInfo.mfa && (<div className="mt-3"><div className="flex gap-2"><input type="password" value={sniperPassword} onChange={e => setSniperPassword(e.target.value)} placeholder="باسورد الحساب" className="flex-1 bg-black/30 border border-cyan-500/20 rounded-lg px-3 py-2 text-xs text-white placeholder-cyan-700/30 focus:outline-none focus:border-cyan-400/40 transition-colors" />{availableNames.length > 0 && (<button onClick={async () => { if (!sniperPassword) { setResult('❌ أدخل الباسورد'); return }; const name = availableNames[0]; setLoading(true); setProgress(`🔄 جاري تغيير اليوزر إلى: ${name}...`); try { const res = await fetch('/api/sniper', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: sniperToken, action: 'changeUsername', targetUsername: name, password: sniperPassword }) }); const data = await res.json(); if (data.success) setResult(`✅ تم تغيير اليوزر إلى: ${name}`); else setResult(`❌ ${data.error}`) } catch { setResult('❌ خطأ') }; setLoading(false); setProgress('') }} className="text-xs text-green-400 bg-green-500/15 px-3 py-2 rounded-lg border border-green-500/25 hover:bg-green-500/25 transition-colors cursor-pointer font-bold">🎯 خذ {availableNames[0]}</button>)}</div></div>)}
                </div>)}
